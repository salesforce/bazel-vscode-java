...
export class BazelSyncStatusProvider implements TreeDataProvider<SyncStatus> {
	private _onDidChangeTreeData: EventEmitter<SyncStatus | undefined | void> =
		new EventEmitter<SyncStatus | undefined | void>();
	readonly onDidChangeTreeData: Event<SyncStatus | undefined | void> =
	...
	if (existsSync(projectPath)) {
		if (lstatSync(projectPath).isDirectory()) {
			for (const x of readdirSync(projectPath)) {
				if (
					lstatSync(path.join(projectPath, x)).isFile() &&
					x.includes('BUILD')
				) {
